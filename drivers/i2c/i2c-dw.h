...
#ifdef CONFIG_MICROKERNEL
typedef enum {
	I2C_NANOKERNEL_SEMAPHORE,
	I2C_MICROKERNEL_SEMAPHORE,
} i2c_sema_type_t;
#endif
...
struct i2c_dw_dev_config {
	/*
	 * In microkernel task waits for the I2C transaction
	 * complete on task_complete_sema while fiber and
	 * initialization task waits complete_sema.
	 * In nanokernel case both fiber and task wait on
	 * complete_sema
	 */
	struct nano_sem		*complete_sema;
#ifdef CONFIG_MICROKERNEL
	ksem_t 			task_complete_sema;
	i2c_sema_type_t		sema_type; /* use micro- or nano- kernel semaphore */
	struct cmd_pkt_set	*sema_cmd_packets;
#endif
