...
static inline void transfer_complete_wait(struct i2c_dw_dev_config * const dw)
{
	nano_context_type_t context = sys_execution_context_type_get();
	kpriority_t prio = task_priority_get();

	/*
	 * If the function is called by a fiber, fiber waits
	 * on nanokernel semaphore.
	 * If function is called by a task, task waits on
	 * microkernel semaphore.
	 * If function is called during the system initialization,
	 * it is called by an idle task. Idle task can't wait
	 * on a microkernel semaphore, it waits on nanokernel
	 * one.
	 */
	if (context == NANO_CTX_TASK &&
	    prio < (CONFIG_NUM_TASK_PRIORITIES - 1)) {
		dw->sema_type = I2C_MICROKERNEL_SEMAPHORE;
	} else {
		dw->sema_type = I2C_NANOKERNEL_SEMAPHORE;
	}
	switch (dw->sema_type) {
	case I2C_NANOKERNEL_SEMAPHORE:
		nano_sem_take_wait(dw->complete_sema);
		break;
	case I2C_MICROKERNEL_SEMAPHORE:
		task_sem_take_wait(dw->task_complete_sema);
		break;
	}
}

static inline void transfer_complete_signal(struct i2c_dw_dev_config * const dw)
{
	switch (dw->sema_type) {
	case I2C_NANOKERNEL_SEMAPHORE:
		nano_isr_sem_give(dw->complete_sema);
		break;
	case I2C_MICROKERNEL_SEMAPHORE:
		isr_sem_give(dw->task_complete_sema, dw->sema_cmd_packets);
		break;
	}
}
...
int i2c_dw_initialize(struct device *port)
{
	...
	nano_sem_init(dev->complete_sema);
...
struct nano_sem i2c_0_complete_sema;

#ifdef CONFIG_MICROKERNEL
static CMD_PKT_SET_INSTANCE(i2c_0_cmd_packets, 2);
struct _k_sem_struct i2c_0_task_complete_sema = __K_SEMAPHORE_DEFAULT;
#endif

struct i2c_dw_dev_config i2c_0_runtime = {
	.complete_sema = &i2c_0_complete_sema,
#ifdef CONFIG_MICROKERNEL
	.task_complete_sema = (ksem_t)&i2c_0_task_complete_sema,
	.sema_cmd_packets = (struct cmd_pkt_set *)i2c_0_cmd_packets,
#endif
