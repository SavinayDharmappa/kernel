 ...
 *
 * @details See misc/Kconfig and the Safe memory access help for details.
 */
...
{
	/* hackish but fast, depends on permissions only using the lsb */
	return !(perm & 0xfffffffe);
}
...
int sys_mem_probe(void *p, int perm, size_t num_bytes, void *buf)
{
...
int sys_mem_safe_read(void *src, char *buf, size_t num_bytes, int width)
{
...
int sys_mem_safe_write(void *dest, char *buf, size_t num_bytes, int width)
{
...
int sys_mem_safe_write_to_text_section(void *dest, char *buf, size_t num_bytes)
{
	ARG_UNUSED(dest);
	ARG_UNUSED(buf);
...
int sys_mem_safe_region_add(void *addr, size_t num_bytes, int perm)
{
	if (unlikely(!is_perm_valid(perm))) {
		return -EINVAL;
	}
...
static int init(struct device *unused)
{
	...
	(void)sys_mem_safe_region_add(addr, num_bytes, SYS_MEM_SAFE_READ);

	addr = (void *)IMAGE_RAM_START;
	num_bytes = (int)(IMAGE_RAM_END - IMAGE_RAM_START);
	(void)sys_mem_safe_region_add(addr, num_bytes, SYS_MEM_SAFE_WRITE);
